...
public class CRC16 {
    ...
    private static byte uniteBytes(byte byte1, byte byte2) {
        byte tempbyte1 = Byte.decode("0x" + new String(new byte[] { byte1 })).byteValue();
        byte tempbyte2 = Byte.decode("0x" + new String(new byte[] { byte2 })).byteValue();
        byte ret = (byte) (((byte) (tempbyte1 << 4)) ^ tempbyte2);
        return ret;
    }

    private static byte[] HexStringToByte(String str) {
        int length = str.length();
        byte[] ret = new byte[length / 2 + 2];
        byte[] tempbyte = str.getBytes();
        for (int i = 0; i < length; i += 2) {
            ret[i / 2] = uniteBytes(tempbyte[i], tempbyte[i + 1]);
        }
        return ret;
    }

    public static byte[] getSendBuf(String str) {
        byte[] tempbyte = HexStringToByte(str);
        CRC16 crc16 = new CRC16();
        crc16.update(tempbyte, tempbyte.length - 2);
        int ri = crc16.getValue();
        tempbyte[tempbyte.length - 1] = (byte) (0xff & ri);
        tempbyte[tempbyte.length - 2] = (byte) ((0xff00 & ri) >> 8);
        return tempbyte;
    }

    public static boolean checkBuf(byte[] bytes) {
        CRC16 crc16 = new CRC16();
        crc16.update(bytes, bytes.length - 2);
        int ri = crc16.getValue();
        if (bytes[bytes.length - 1] == (byte) (ri & 0xff)
